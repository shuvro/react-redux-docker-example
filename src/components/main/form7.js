...
import { getFormData, postFormData, updateFormFieldValue, redirectToUrl, autoPostFormData, deleteImage } from '../../actions'
import { Row, Col, Form, Button } from 'react-bootstrap';
import ROUTES from '../../utils/routes';
import { InputField, SelectField, TextAreaField, SwitchField, CheckboxField, UploadField, SignaturePadField, DateField, UploadFieldDropzone, UploadDropzoneComponent } from '../fields';
import ReactHtmlParser from 'react-html-parser';
import language from '../../constants/error-msg-definitions';

class Form7Component extends Component {
  STEP = 7;
  state = {
    blocks: [],
    form: {},
    ...
    if (form[this.STEP].length === 0 || Object.keys(fieldsError).length === 0) {
      ...
      if (config.form.showNavigation) {
        setTimeout(() => {
          ...
          styleElemBefore.innerHTML = `.camCon.active::before { border-color: ${config.baseColor};}`;
        }, 30);
      }
      return (
        <div>
          {config.form.showNavigation &&
            <div className="progress-block">
              <ul className="list-unstyled multi-steps">
                ...
                {/*<li onClick={e => this.gotoStep(e, ROUTES.FORM5)} ></li>*/}
                ...
                <li></li>
                <li></li>
                <li className="camCon active">
                  <svg className="maroon-icon" xmlns="http://www.w3.org/2000/svg" width="17" height="16" viewBox="0 0 17 16"><g><g><path fill={config.baseColor} d="M5.818 8.556a2.738 2.738 0 1 1 5.475 0 2.738 2.738 0 0 1-5.475 0z" /></g><g><path fill={config.baseColor} d="M8.556 12.833a4.28 4.28 0 0 1-4.278-4.278 4.28 4.28 0 0 1 4.278-4.277 4.28 4.28 0 0 1 4.277 4.277 4.28 4.28 0 0 1-4.278 4.278zM5.989 0L4.423 1.711H1.711C.77 1.711 0 2.481 0 3.422V13.69c0 .94.77 1.71 1.711 1.71H15.4c.941 0 1.711-.77 1.711-1.71V3.422c0-.94-.77-1.71-1.711-1.71h-2.712L11.122 0z" /></g></g></svg>
                </li>
              </ul>
            </div>
          }
                      ...
                      <Row style={{ borderLeft: `7px solid ${config.baseColor}` }} className={blockItem.block.class === 'info' ? 'light-blue-section' : blockItem.block.class === 'warning' ? 'light-warning-section' : ''}>
                        ...
                        {blockItem.block.fields && blockItem.block.fields.map((field, fieldIndex) => {
                          ...
                          switch (field.type) {
                            ...
                            case 'UPLOAD':
                              return <UploadFieldDropzone {...field} key={fieldIndex} onChange={this.onChange} error={fieldsError[field.name]} lang={config.form.settings.lang}
                                remoteUrl={config.form.remoteUrl + '/tmp/kyc/'} deleteImage={this.props.deleteImage} step={this.STEP} blockIndex={bIndex} getFormData={this.props.getFormData}/>
                            case 'SIGNATUREPAD':
