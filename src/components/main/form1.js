...
class Form1Component extends Component {
  ...
  onPartnerDelete = (id) => {
    this.props.deletePartner({ id: id });
  }
            ...
            {blocks.map((blockItem, bIndex) => {
              if (['hide', 'show'].includes(blockItem.block.display)) {
                if (blockItem.block.trigger) {
                  blockItem.block.display = this.state.form[blockItem.block.trigger] === 'Otro' ? 'show' : 'hide'
                } else {
                ...
                return (
                  <div className='form-div' key={bIndex}>
                    ...
                    <Row style={{ borderLeft: `7px solid ${config.baseColor}` }}
                         className={blockItem.block.class === 'info' ? 'light-blue-section' : blockItem.block.class === 'warning' ? 'light-warning-section' : bIndex === 5 ? 'div-width' : ''}>
                      ...
                      {blockItem.block.fields && blockItem.block.fields.map((field, fieldIndex) => {
                        if (['hide', 'show'].includes(field.display) && field.action) {
                          field.display = this.state.form[field.action[1]] ? 'hide' : 'show'
                        } else {
                          field.display = 'show'
                        }
                        switch (field.type) {
                          ...
                          case 'SWITCH':
                            return <SwitchField {...field} key={fieldIndex} onChange={this.onChange}
                                                error={fieldsError[field.name]} lang={config.form.settings.lang} applyStyle="1"/>
                          case 'CHECKBOX':
                          ...
                          case 'TABLE':
                            return <TableField {...field} key={fieldIndex} onChange={this.onChange}
                                               error={fieldsError[field.name]} lang={config.form.settings.lang} addOrEdit={this.props.redirectToUrl} deletePartner={this.onPartnerDelete}/>
                          default:
                            return <InputField {...field} key={fieldIndex} onChange={this.onChange}
                                               error={fieldsError[field.name]} lang={config.form.settings.lang}/>
                        }
