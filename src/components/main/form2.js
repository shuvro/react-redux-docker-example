...
class Form2Component extends Component {
  ...
  state = {
    ...
    formLength: this.props.form[this.STEP].length ? this.props.form[this.STEP].length: 5
  }
  ...
  updateStateForm = () => {
    if (this.props.form[this.STEP]) {
      ...
      this.setState({ form: formList, fieldsError: fieldsErrorList, formLength: this.props.form[this.STEP].length })
    }
  }
  ...
  onChange = (e, name, type, validation, blockIndex = 0, mask = '') => {
    ...
    this.props.updateRepresentativeFormFieldValue({
      step: this.STEP,
      field: name,
      type,
      value: form[name],
  ...
  validateRepresentativeBlocks = (e, tabs) => {
    e.preventDefault()
    let valid = true
    let { fieldsError: fieldsErrorList, form: formList, formLength: formLength } = this.state
    const { config } = this.props
    const emailValidator = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/
    fieldsErrorList.forEach((fieldsError, index) => {
      let form = formList[index]
    ...
    if (!valid) {
      ...
      this.setState({formLength: formLength + 4})
      this.props.cloneRepresentative(tabs)
    }
  }
  ...
  makeButtonDisable = () => {
    return this.state.formLength >= 41;
  }
        ...
        <div>
          ...
          <Form onSubmit={this.onFormSubmit}>
              ...
              if (blockItem.block.display === 'show' && bIndex === 0) {
                ...
                return (
                  <div className='form-div' key={bIndex}>
                    ...
                    {blockItem.block.eliminate && bIndex !== 1 &&
                    <Button variant="outline-secondary" onClick={e => this.props.removeRepresentative(bIndex)}
                            style={buttonRemove}>Eliminar</Button>}
                        ...
                        switch (field.type) {
                          case 'text':
                            return <InputField blockIndex={bIndex} {...field} key={fieldIndex} onChange={this.onChange}
                                               error={fieldsError[bIndex - 1][field.name]}
            ...
            <Row>
              <Col xs={0} md={4}>
                <Row>
                  <h5 className="form-block-title">Desea agregar otro representante?</h5>
                </Row>
              </Col>
              <Col xs={12} md={8}>
                <Row>
                  <Col xs={6} md={4}>
                    <Button variant="outline-secondary" disabled={this.makeButtonDisable()} {...this.formIsValid()} onClick={e => this.validateRepresentativeBlocks(e,tabs)}
                            style={buttonClone}>Si</Button>
                  </Col>
                  <Col xs={12} md={4}>
