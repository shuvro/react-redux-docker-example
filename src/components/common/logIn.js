...
import { getLoginAtrributes, postLoginForm, isLoggedInAndRedirect } from '../../actions';
...
class LogIn extends Component {
  ...
  componentWillMount() {
    ...
    if (!this.props.auth.atrributes) {
      this.props.getLoginAtrributes();
    } else {
  ...
  updateStateForm = () => {
    if (this.props.auth.atrributes !== null) {
      let form = {};
      let requiredFields = [];
      let fieldsError = {};
      const atrributes = this.props.auth.atrributes;
  ...
  render() {
    ...
    if (auth.atrributes === null || Object.keys(fieldsError).length === 0) {
      return <div></div>
    } else {
      const atrributes = auth.atrributes;
                  ...
                  <Form onSubmit={this.submitForm}>
                    <Row>
                      ...
                      {atrribute.block.fields.map((field, fieldIndex) => (
                        <InputField {...field} key={fieldIndex} onChange={this.onChange} error={fieldsError[field.name]} display='show' lang={config.form.settings.lang} />
                      ))
